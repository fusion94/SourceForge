#!/usr/bin/perl
#
# $Id: db_top_groups_calc.pl,v 1.29 2000/05/05 20:24:27 dtype Exp $
#
use DBI;
...
my $query = "SELECT group_id,group_name FROM groups";
...
my $query = "SELECT group_id,SUM(downloads) AS count FROM frs_dlstats_group_agg "
	."WHERE day>=$oneweekago_fmt GROUP BY group_id ORDER BY count DESC";
my $rel = $dbh->prepare($query);
$rel->execute();
$currentrank = 1;
while(my ($group_id,$count) = $rel->fetchrow()) {
